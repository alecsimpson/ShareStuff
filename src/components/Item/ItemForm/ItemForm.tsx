...
export default function ItemForm({ item, onCancel, onSave }: ItemFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white border rounded-xl p-4 sm:p-5 shadow-sm space-y-4">
      ...
      <div className="text-xs text-gray-500">
        <div>Added by: <span className="font-medium text-gray-700">{item.addedBy}</span></div>
        <div>Added at: <span className="font-medium text-gray-700">{new Date(item.addedAt).toLocaleString()}</span></div>
      </div>
